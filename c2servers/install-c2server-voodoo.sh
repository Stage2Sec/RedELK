...
#
# Part of RedELK
# Script to install RedELK on Voodoo servers
#
# Author: Outflank B.V. / Marc Smeets / @benpturner - Ben Turner / @PaulWhitingS2
#

LOGFILE="redelk-install.log"
INSTALLER="RedELK Voodoo installer"
ELKVERSION="7.9.2"
ELASTICVERSION="7.x"
...
echo ""
echo ""   
echo "This script will install and configure necessary components for RedELK on Voodoo teamservers"
echo ""
echo ""
...
printf 'LANG=en_US.UTF-8\nLC_ALL=en_US.UTF-8\n' > sudo tee /etc/default/locale >> $LOGFILE 2>&1
...
sudo apt-get install -y apt-transport-https >> $LOGFILE 2>&1
ERROR=$?
if [ $ERROR -ne 0 ]; then
...
if [ ! -f  /etc/apt/sources.list.d/elastic-$ELASTICVERSION.list ]; then
    echo "deb https://artifacts.elastic.co/packages/$ELASTICVERSION/apt stable main" | sudo tee -a /etc/apt/sources.list.d/elastic-$ELASTICVERSION.list >> $LOGFILE 2>&1
fi
ERROR=$?
if [ $ERROR -ne 0 ]; then
...
sudo apt-get install -y filebeat=$ELKVERSION >> $LOGFILE 2>&1
ERROR=$?
if [ $ERROR -ne 0 ]; then
...
sudo systemctl enable filebeat >> $LOGFILE 2>&1
ERROR=$?
if [ $ERROR -ne 0 ]; then
...
sudo mv /etc/filebeat/filebeat.yml /etc/filebeat/filebeat.yml.ori >> $LOGFILE 2>&1
ERROR=$?
if [ $ERROR -ne 0 ]; then
...
sudo cp ./filebeat/filebeat_voodoo.yml /etc/filebeat/filebeat.yml >> $LOGFILE 2>&1
ERROR=$?
if [ $ERROR -ne 0 ]; then
...
sudo cp ./filebeat/redelkCA.crt /etc/filebeat/ >> $LOGFILE 2>&1
ERROR=$?
if [ $ERROR -ne 0 ]; then
...
sudo sed -i s/'@@HOSTNAME@@'/$1/g /etc/filebeat/filebeat.yml  >> $LOGFILE 2>&1
ERROR=$?
if [ $ERROR -ne 0 ]; then
...
sudo sed -i s/'@@ATTACKSCENARIO@@'/$2/g /etc/filebeat/filebeat.yml >> $LOGFILE 2>&1
ERROR=$?
if [ $ERROR -ne 0 ]; then
...
sudo sed -i s/'@@HOSTANDPORT@@'/$3/g /etc/filebeat/filebeat.yml >> $LOGFILE 2>&1
ERROR=$?
if [ $ERROR -ne 0 ]; then
...
lp_dir=$(echo "$4" | sed 's/\//\\\//g')
sudo sed -i s/'@@VOODOO_LP_DIR@@'/$lp_dir/g /etc/filebeat/filebeat.yml >> $LOGFILE 2>&1
ERROR=$?
if [ $ERROR -ne 0 ]; then
    echoerror "[X] Could not change log destination field in filebeat config (Error Code: $ERROR)."
fi
...
sudo service filebeat start >> $LOGFILE 2>&1
ERROR=$?
if [ $ERROR -ne 0 ]; then
...
sudo mkdir -p /var/log/redelk >> $LOGFILE 2>&1
ERROR=$?
if [ $ERROR -ne 0 ]; then
...
sudo service filebeat restart >> $LOGFILE 2>&1
ERROR=$?
if [ $ERROR -ne 0 ]; then
...
echo ""
echo ""
echo "Done with setup of RedELK on Voodoo server."
echo ""
